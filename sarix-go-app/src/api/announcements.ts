 ...
 *
 * Push alone could never reach everyone: it needs a registered token on a real device
 * with notification permission, so passengers who joined through the Telegram bot got
 * nothing. The inbox is readable by any signed-in account, which is what makes an admin
 * broadcast actually arrive.
 */
