export default {
  ...
  faq: {
    ...
    a9: 'После завершения поездки вы оцениваете водителя от 1 до 5 звёзд, а водитель оценивает вас. Оценка необязательна — её можно пропустить кнопкой «Позже».\n\nРейтинг — это простое среднее всех полученных звёзд, показанное с одним знаком после запятой (например 4.8). За одну поездку ставится одна оценка, и её нельзя изменить после отправки. Оценивать можно только завершённые поездки — отменённый заказ или заказ, на который не нашёлся водитель, оценить нельзя.\n\nКомментарий тоже необязателен, до 500 символов.',
  },
  driverMap: {
    driverPin: 'Водитель',
    pickupPin: 'Посадка',
